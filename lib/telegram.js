 ...
 */
var TelegramBot = function TelegramBot(token, options) {
  options = options || {};
  this.options = options;
  this.token = token;
  this.messageTypes = ['text', 'audio', 'document', 'photo', 'sticker', 'video', 'voice', 'contact', 'location', 'new_chat_participant', 'left_chat_participant', 'new_chat_title', 'new_chat_photo', 'delete_chat_photo', 'group_chat_created', 'callback_query']; // Telegram message events
...
TelegramBot.prototype._processUpdate = function (update) {
  ...
  if (message) {
    ...
    debug('Process Update callback_query %j', callback_query);
